...
class SwiftSnapshot:
    def __init__(
        ...
        self.max_size_to_read_at_once = 2 * 1024.0 * 1024 * 1024  # 2 Gb in bytes
    ...
    def split_selection_distributed(self):
        ...
        if self.verbose:
            self.message("Split selection is distributed.")
        assert (
            self.region_selected_on != -1
        ...
        for i, fileno in enumerate(np.unique(self.region_data["files"])):
            ...
            self.index_data["total_num_to_load"] += np.sum(
                self.index_data["num_to_load"][-1]
            )
    ...
    def split_selection_collective(self):
        ...
        if self.verbose:
            self.message("Split selection is collective.")
        assert (
            self.region_selected_on != -1
        ), "Need to call select region before split_selection"
        ...
        if (
            self.comm_size > 1
            and self.region_data["total_num_to_load"] > 100 * self.comm_size
        ):
            ...
            for fileno in np.unique(self.region_data["files"]):
                ...
                if num_per_core[0] < 1e3:
                    if self.comm_rank == self.comm_size - 1:
                        if self.verbose:
                            print(
                                "Rank %i will load %i particles file=%i"
                                % (self.comm_rank, num_to_load_from_this_file, fileno)
                            )

                        self.index_data["lefts"].append(
                            self.region_data["lefts"][file_mask]
                        )
                        self.index_data["rights"].append(
                ...
                # The case where this file has many particles per rank.
                    ...
                    if self.verbose:
                        self.comm.barrier()
                        print(
                            "Rank %i will load %i particles l=%s r=%s from file=%i"
                            % (
                                self.comm_rank,
                                num_per_core[self.comm_rank],
                                tmp_my_lefts,
                                tmp_my_rights,
                                fileno,
                            )
                    ...
                    self.index_data["files"].append(fileno)
                    self.index_data["num_to_load"].append(
                        np.array(self.index_data["rights"][-1])
                        - np.array(self.index_data["lefts"][-1])
                    )
                    self.index_data["total_num_to_load"] += np.sum(
                        self.index_data["num_to_load"][-1]
                    )
    ...
    def find_select_region_cells(
        self, f, x_min, x_max, y_min, y_max, z_min, z_max, eps=1e-4
    ):
        ...
        centres = (
            np.mod(centres - coords + 0.5 * boxsize, boxsize) + coords - 0.5 * boxsize
        )
    ...
    def select_region(
        self, part_type, x_min, x_max, y_min, y_max, z_min, z_max, just_load_all=False
    ):
        ...
        if self.comm_rank == 0:
            ...
            if self.verbose:
                print(
                    "Selection region x=%.4f->%.4f y=%.4f->%.4f z=%.4f->%.4f PT=%i"
            ...
            if "Cells" in f:
                mask = self.find_select_region_cells(
        ...
        if just_load_all:
            if self.verbose:
                self.message("Just loading all cells.")
            ...
            if self.verbose:
                self.message("Indexing files.")
            lo_ranks = np.arange(
                0, self.comm_size + self.max_concur_io, self.max_concur_io
            )[:-1]
            hi_ranks = np.arange(
                0, self.comm_size + self.max_concur_io, self.max_concur_io
            )[1:]
            for lo, hi in zip(lo_ranks, hi_ranks):
                for this_file_i in range(self.HEADER["NumFilesPerSnapshot"]):
                    if not (self.comm_rank >= lo and self.comm_rank < hi):
                        continue

                    if self.comm_size > 1:
                        if this_file_i % self.comm_size != self.comm_rank:
                            continue
                    tmp_f = h5py.File(self.get_filename(this_file_i), "r")
                    tmp_num_this_file = tmp_f["Header"].attrs["NumPart_ThisFile"][
                        part_type
                    ]
            ...
            self.region_data["total_num_to_load"] = self.HEADER["NumPart_Total"][
                part_type
            ]
            for att in ["lefts", "rights", "num_to_load", "files"]:
                if self.comm_size > 1:
                    self.region_data[att] = np.concatenate(
                        self.comm.allgather(self.region_data[att])
                    )
                self.region_data[att] = np.array(self.region_data[att], dtype="i8")

            # Make sure we found all the particles we intended to.
            assert (
                np.sum(self.region_data["num_to_load"])
                == self.region_data["total_num_to_load"]
        ...
        # We have top level cell information, just load selected region.
            ...
            if self.comm_rank == 0:
                ...
                if self.verbose and self.comm_rank == 0:
                    print(
                        "%i cells selected from %i file(s)."
                        % (len(offsets), len(np.unique(files)))
                    )
                    ...
                    for i in range(len(offsets) - 1):
                        if offsets[i] + counts[i] == offsets[i + 1]:
                            ...
                            if i == len(offsets) - 2:
                                ...
                                self.region_data["rights"].append(
    ...
    def read_dataset_distributed(self, parttype, att, physical_cgs):
        ...
        if self.verbose:
            print(
                "[Rank %i] Loading %i particles from %i files."
                % (self.comm_rank, tot, num_files)
            )
        ...
        for lo, hi in zip(lo_ranks, hi_ranks):
            ...
            for j, fileno in enumerate(self.index_data["files"]):
                ...
                for l, r in zip(
                    self.index_data["lefts"][j], self.index_data["rights"][j]
                ):
                    this_count = r - l
                    ...
                    num_chunks = int(
                        np.ceil(
                            (this_count * byte_size) / self.max_size_to_read_at_once
                        )
                    )
    ...
    def read_dataset_collective(self, parttype, att, physical_cgs):
        ...
        for j, fileno in enumerate(self.index_data["files"]):
            ...
            if (
                ...
                if self.verbose:
                    self.message(
                        "[%s] Collective loading %i particles from file %i."
                        % (att, tot, fileno)
                    )
            else:
                ...
                if self.verbose:
                    print(
                        "[%s] Serial loading %i particles from file %i."
                        % (att, tot, fileno)
                    )
            ...
            if (
                ...
                f = h5py.File(
                    self.get_filename(fileno), "r", driver="mpio", comm=self.comm
                )
                f.atomic = True
            else:
                f = h5py.File(self.get_filename(fileno), "r")
            ...
            if j == 0:
                ...
                if len(shape) > 1:
                    return_array = np.empty(
                        (np.sum(self.index_data["total_num_to_load"]), shape[1]),
                        dtype=dtype,
                    )
                    byte_size = dtype.itemsize * shape[1]
                else:
                    byte_size = dtype.itemsize
                    return_array = np.empty(
                        np.sum(self.index_data["total_num_to_load"]), dtype=dtype
                    )
                return_array = return_array.astype(return_array.dtype.newbyteorder("="))

                # What do we multiply by to get physical cgs?
                if physical_cgs:
                    attr_name = "Conversion factor to physical CGS (including cosmological corrections)"
                    physical_fac = f["PartType%i/%s" % (parttype, att)].attrs.get(
                        attr_name
                    )
            ...
            for l, r in zip(self.index_data["lefts"][j], self.index_data["rights"][j]):
                ...
                # Need to chunk it.
                num_chunks = int(
                    np.ceil((this_count * byte_size) / self.max_size_to_read_at_once)
                )
                num_per_cycle = np.tile(this_count // num_chunks, num_chunks)
                mini_lefts = np.cumsum(num_per_cycle) - num_per_cycle[0]
                mini_rights = np.cumsum(num_per_cycle)
        ...
        if self.comm_size > 1:
            return_array_dtype = self.comm.bcast(return_array_dtype)
    ...
    def _get_hdf5_attr(self, f, attr_group, attr):
        """ Read an HDF5 attribute. """

        tmp = f[attr_group].attrs.get(attr)

        # Convert arrays with length 1.
    ...
    def read_header(self):
        ...
        for grp in ["Header", "Cosmology", "Parameters"]:
            ...
            setattr(self, grp.upper(), {})
            for att in f[grp].attrs.keys():
                getattr(self, grp.upper())[att] = self._get_hdf5_attr(f, grp, att)

        # Convert total number of particles to unsigned I64.
        if "NumPart_Total" in self.HEADER.keys():
            self.HEADER["NumPart_Total"] = np.array(
                self.HEADER["NumPart_Total"], dtype=np.uint64
            )
        ...
        if "BoxSize" in self.HEADER.keys():
            if (
                type(self.HEADER["BoxSize"]) == list
                or type(self.HEADER["BoxSize"]) == np.ndarray
            ):
